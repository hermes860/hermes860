...
def main():
    ...
    while True:
        ...
        if eth_proto == 8:
            ...
            print ('\tSource Adress             : {0}'.format(src))
            ...
            #TCP
            if protocol == 6:
                src_port, dest_port, sequence, acknowledgment, flag_urg, flag_ack, flag_psh, flag_rst, flag_syn, flag_fin = struct.unpack('! H H L L H H H H H H', read_data[:24])
...
def tcp_seq(packet_data):
    (src_port, destination_port, sequence, acknowledgement, offset_reserv_flag) = struct.unpack('! H H L L H',packet_data[:14])
    ...
    flag_sym = (offset_reserved_flag & 32) >> 1
